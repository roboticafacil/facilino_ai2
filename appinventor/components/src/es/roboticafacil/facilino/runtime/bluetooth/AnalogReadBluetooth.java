...
public class AnalogReadBluetooth  extends AnalogReadBase implements FacilinoBluetoothSensor {
	 ...
	 */
	public AnalogReadBluetooth(ComponentContainer container) {
		super(container);
	}
	
	protected AnalogReadBluetooth(ComponentContainer container, String logTag, byte type)
	{
	...
	private YailList readTelegram() {
		...
		bytes[0]='@';
		bytes[1]=FacilinoBluetoothClient.CMD_ANALOG_READ_REQ;
		bytes[2]=1;
		bytes[3]=(byte)(_pin);
		bytes[4]='*';
		int n=bytes.length;
	...
	public void dispatchData(byte cmd,byte[] data) {
		if (cmd==FacilinoBluetoothClient.CMD_ANALOG_READ_RESP){
			if (data[0]==_pin)
			{

...
public class RGB_LEDStripBluetooth  extends RGB_LEDStripBase implements FacilinoBluetoothActuator {
   ...
   */
  public RGB_LEDStripBluetooth(ComponentContainer container) {
    super(container);
  }
  ...
  public void SetLEDs(YailList colors) {
	  if (_facilino instanceof FacilinoBluetoothClient)
			((FacilinoBluetoothClient)_facilino).SendBytes(setLEDsTelegram(colors));
  }
  ...
  public void SetBrightness(int number) {
	  if (_facilino instanceof FacilinoBluetoothClient)
			((FacilinoBluetoothClient)_facilino).SendBytes(setBrightnessTelegram(number));
  }
  ...
  private YailList setLEDsTelegram(YailList colors)
  {
	  ...
	  bytes[0]='@';
	  bytes[1]=FacilinoBluetoothClient.CMD_LED_STRIP;
  ...
  private YailList setBrightnessTelegram(int number)
  {
	  ...
	  bytes[0]='@';
	  bytes[1]=FacilinoBluetoothClient.CMD_LED_STRIP_SET_BRIGHTNESS;
	  bytes[2]=2;
	  bytes[3]=(byte)_pin;
	  bytes[4]=(byte) number;
	  bytes[5]='*';

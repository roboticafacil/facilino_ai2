...
public class StringVariableBluetooth  extends StringVariableBase implements FacilinoBluetoothSensorActuator {
	/**
	 * Creates a new Facilino component.
	 */
	public StringVariableBluetooth(ComponentContainer container) {
		super(container);
	}
	...
	private YailList readTelegram() {
		...
		bytes[0]='@';
		bytes[1]=FacilinoBluetoothClient.CMD_STRING_VAR_READ_REQ;
		bytes[2]=1;
		bytes[3]=(byte)_index;
		bytes[4]='*';
		int n=bytes.length;
	...
	public void dispatchData(byte cmd, byte[] data) {
		if (cmd==FacilinoBluetoothClient.CMD_STRING_VAR_READ_RESP){
			if (data[0]==_index)
			{
				int len=data[1];
	...
	private YailList setTelegram(String value)
	{
		...
		bytes[0]='@';
		bytes[1]=FacilinoBluetoothClient.CMD_STRING_VAR_WRITE_REQ;
		bytes[2]=(byte)(2+len);
		bytes[3]=(byte)_index;
		bytes[4]=(byte)len;
		for (int i=0;i<len;i++)

...
import es.roboticafacil.facilino.runtime.bluetooth.ServoBase;
...
public class ServoBluetooth  extends ServoBase implements FacilinoBluetoothActuator {
  /**
   * Creates a new Facilino component.
   */
  public ServoBluetooth(ComponentContainer container) {
    super(container);
  }
  ...
  public void Move(byte angle) {
	  if (_facilino instanceof FacilinoBluetoothClient)
			((FacilinoBluetoothClient)_facilino).SendBytes(moveTelegram(angle));
  }
  
  private YailList moveTelegram(byte angle) {
	  byte[] bytes = new byte[6];
	  bytes[0]='@';
	  bytes[1]=FacilinoBluetoothClient.CMD_SERVO;
	  bytes[2]=2;
	  bytes[3]=(byte)_pin;
	  bytes[4]=(byte)angle;
	  bytes[5]='*';

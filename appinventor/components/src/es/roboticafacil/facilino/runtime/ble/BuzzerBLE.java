...
import es.roboticafacil.facilino.runtime.ble.BuzzerBase;
...
public class BuzzerBLE  extends BuzzerBase implements FacilinoBLEActuator {
	
	/**
	 * Creates a new Facilino component.
	 */
	public BuzzerBLE(ComponentContainer container) {
		super(container);
	}
	...
	public void Tone(int frequency, int duration) {
		if (_ble instanceof FacilinoBLEClient)
			((FacilinoBLEClient)_ble).SendBytes(toneTelegram(frequency,duration));
	}
	...
	public void Song(int number) {
		if (_ble instanceof FacilinoBLEClient)
			((FacilinoBLEClient)_ble).SendBytes(predefMelodyTelegram(number));
	}

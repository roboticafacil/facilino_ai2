...
import es.roboticafacil.facilino.runtime.ble.ServoContBase;
...
public class ServoContBLE  extends ServoContBase implements FacilinoBLEActuator {
  /**
   * Creates a new Facilino component.
   */
  public ServoContBLE(ComponentContainer container) {
    super(container);
  }
  ...
  public void Move(int velocity) {
	  if (_ble instanceof FacilinoBLEClient)
			((FacilinoBLEClient)_ble).SendBytes(moveTelegram(velocity));
  }
  ...
  private YailList moveTelegram(int velocity)
  {
	  ...
	  bytes[0]='@';
	  bytes[1]=FacilinoBase.CMD_SERVO_CONT;
	  bytes[2]=2;
	  bytes[3]=(byte)_pin;
	  bytes[4]=_servoCmd;
	  bytes[5]='*';

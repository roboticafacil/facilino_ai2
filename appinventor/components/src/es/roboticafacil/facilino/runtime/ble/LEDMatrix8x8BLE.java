...
import es.roboticafacil.facilino.runtime.ble.LEDMatrix8x8Base;
...
public class LEDMatrix8x8BLE  extends LEDMatrix8x8Base implements FacilinoBLEActuator {
  /**
   * Creates a new Facilino component.
   */
  public LEDMatrix8x8BLE(ComponentContainer container) {
    super(container); 
  }
  ...
  public void ShowCustomExpression(byte col1, byte col2, byte col3, byte col4, byte col5, byte col6, byte col7, byte col8) {
		if (_ble instanceof FacilinoBLEClient)
			((FacilinoBLEClient)_ble).SendBytes(showTelegram(col1,col2,col3,col4,col5,col6,col7,col8));
  }
  ...
  public void ShowPredefExpression(byte number) {
	  if (_ble instanceof FacilinoBLEClient)
			((FacilinoBLEClient)_ble).SendBytes(showPredefExpressionTelegram(number));
  }

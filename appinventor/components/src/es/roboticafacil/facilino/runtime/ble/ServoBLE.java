...
import es.roboticafacil.facilino.runtime.ble.ServoBase;
...
public class ServoBLE  extends ServoBase implements FacilinoBLEActuator {
  /**
   * Creates a new Facilino component.
   */
  public ServoBLE(ComponentContainer container) {
    super(container);
  }
  ...
  public void Move(byte angle) {
	  if (_ble instanceof FacilinoBLEClient)
			((FacilinoBLEClient)_ble).SendBytes(moveTelegram(angle));
  }
  
  private YailList moveTelegram(byte angle) {
	  byte[] bytes = new byte[6];
	  bytes[0]='@';
	  bytes[1]=FacilinoBase.CMD_SERVO;
	  bytes[2]=2;
	  bytes[3]=(byte)_pin;
	  bytes[4]=(byte)angle;
	  bytes[5]='*';
